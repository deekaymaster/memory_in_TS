...
const checkPair = (cards) => {
    let pairToCheck = [];
    cards.forEach(card => {
        if(card.classList.contains("picked")) pairToCheck.push(card);
    });
...
const lockPicking = () => {
    ...
    let picked=0;
    cards.forEach(card => {
        if(card.classList.contains("picked")) picked++;
        if(picked>=2)
        {
...
const pickUpCard = (cardElement: HTMLDivElement) => {
...
let renderBoard = (size: number) => {
    ...
    cardsArray.forEach(card => {
        ...
        if(size == 3) cardElement.classList.add('threeXfour');
        else if(size == 4) cardElement.classList.add('fourXfour')
